...
\begin{figure}[ht]
 ...
 \caption{\small Sample initial Wollok \carlostext{object definition}. \carlostext{\cl{OJO agregué un método en todas las versiones.}}
}
...
The first tool we introduce to try out a program without requiring I/O is a \emph {read-eval-print-loop} (REPL). Running a program in the REPL brings all defined objects to live and allows the user to interact with them sending messages, as shown in Fig.~\ref{fig:repl}. 
...
\begin{figure}[ht]
 ...
 \caption{\small Sample usage of the accumulator \carlostext{object} in the REPL}
 \label{fig:repl}
\end{figure}
...
\carlostext{In particular, the test runner guarantees that there is no interaction between tests: the messages sent to the accumulator in the first test will not affect the state of the accumulator in the second test\np{sería lindo tener una cita a alguna regla de unit testing que proponga esto}.

Furthermore, tests have to be written in a separate file.
On the other hand, the objects (and later on, classes) defined in a different file must be accesible from the test file. To this end, Wollok includes a simple form of the \texttt{\textbf{import}} directive, that refers to a object/class definition file in the same folder than the test file. In this way, some basic knowledge of modularization is subtly introduced at the very beginning of the course\footnote{\carlostext{The assimilation of the \texttt{import} directive, as early as in the second week, was not problematic in our experience using Wollok for first-year programming courses.}}.}
...
This allows us to \carlostext{use collections, and therefore} increase the complexity of examples that we can build \carlostext{before}\cl{aca decia ``without''} introducing classes.
\carlostext{We even briefly introduce \emph{closures} at the initial stage of the course.}
...
\carlostext{Moreover, methods not including a \texttt{return} expression are considered as void, i.e. they do not yield a value and no operation can be applied to them. Furthermore, the REPL do not show any text as result of the evaluation of void methods, as can be seen in Fig.~\ref{fig:repl}.}\cl{esta frase hay que pulirla.}
One step forward is to incorporate an \emph{effect system} \cite{nielson_type_1999}, \cf Sec. \ref{sec:furtherWork}. \cl{tal vez convenga mover la oración entera, desde ``one step forward'', al final.}
